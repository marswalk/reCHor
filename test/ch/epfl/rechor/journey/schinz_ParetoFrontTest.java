...
class schinz_ParetoFrontTest {
    ...
    void paretoFrontBuilderAddWorksLikeNaiveParetoFrontBuilder() {
        ...
        for (int i = 0; i < 1000; i += 1) {
            ...
            System.out.println("\n\n\n\n\t===ITERATION " + i + "===\nAdding tuple: " + formatTuple(t));
            naiveParetoFrontB.add(t);
            System.out.println("Our builder:");
            paretoFrontB.add(t);
            compareBuilders(paretoFrontB, naiveParetoFrontB);
    ...
    static class NaiveParetoFrontBuilder {
        ...
        public void add(long newTuple) {
            System.out.println("\n===== NaiveBuilder: Adding tuple: " + formatTuple(newTuple) + " =====");
            ...
            while (tupleIt.hasNext()) {
                ...
                System.out.println("Comparing: " + formatTuple(tuple) + " with new tuple: " + formatTuple(newTuple));

                if (PackedCriteria.dominatesOrIsEqual(tuple, newTuple)) {
                    System.out.println("New tuple is dominated by existing tuple: " + formatTuple(tuple));
                    return;
                }

                if (PackedCriteria.dominatesOrIsEqual(newTuple, tuple)) {
                    System.out.println("Tuple " + formatTuple(tuple) + " is dominated and will be removed");
                    tupleIt.remove();
                }
            }
            tuples.add(newTuple);
            System.out.println("Added new tuple to front");
        ...
        private String debugArrayToString() {
            ...
            for (int i = 0; i < tuples.size(); i++) {
                sb.append(formatTuple(tuples.get(i)));
                if (i < tuples.size() - 1) sb.append(", ");
            }
            return sb.append("] (size: ").append(tuples.size()).append(")").toString();
        }
        ...
        public String tupleToString(long pTuple) {
    ...
    private static void compareBuilders(ParetoFront.Builder paretoFrontB, NaiveParetoFrontBuilder naiveParetoFrontB) {
        List<Long> expected = naiveParetoFrontB.toSortedList();
        List<Long> actual = toSortedList(paretoFrontB);

        System.out.println("\n===== COMPARISON =====");
        System.out.println("Naive builder contents:");
        if (expected.isEmpty()) {
            System.out.println("  [empty]");
        } else {
            for (Long tuple : expected) {
                System.out.println("  • " + formatTuple(tuple));
            }
        }

        System.out.println("Actual builder contents:");
        if (actual.isEmpty()) {
            System.out.println("  [empty]");
        } else {
            for (Long tuple : actual) {
                System.out.println("  • " + formatTuple(tuple));
            }
        }

        if (expected.equals(actual)) {
            System.out.println("MATCH: Both builders contain the same elements");
        } else {
            System.out.println("MISMATCH: Builders contain different elements!");
            System.out.println("Expected size: " + expected.size() + ", Actual size: " + actual.size());

            // Find elements in expected but not in actual
            for (Long tuple : expected) {
                if (!actual.contains(tuple)) {
                    System.out.println("Missing from actual: " + formatTuple(tuple));
                }
            }

            // Find elements in actual but not in expected
            for (Long tuple : actual) {
                if (!expected.contains(tuple)) {
                    System.out.println("Extra in actual: " + formatTuple(tuple));
                }
            }
        }
    }
